...
- **Algorithme** : RSA 4096
- **Expiration** : aucune (rotation manuelle si besoin, cf. ADR `0003-backup-encryption-asymmetric-gpg.md`)
...
Lire l'ADR `0003` — c'est un risque assumé. Conséquence : les backups existants chiffrés avec cette pub key deviennent définitivement illisibles. Procédure de récupération : générer une nouvelle keypair, remplacer `aurore-backup.pub.asc`, ré-importer sur le VPS, démarrer une nouvelle génération de backups. Les anciens dumps sont perdus.
