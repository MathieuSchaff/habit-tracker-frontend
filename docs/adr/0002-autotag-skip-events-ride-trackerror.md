...
- `errorGroups` now contains non-crash entries. Filterable via `message = 'product_autotag_skipped'`, queryable by `userId` (top-level FK) and `productId` (in `context` jsonb). The constant is exported from the helper module so query call sites do not drift.
...
- The asymmetry between the two helpers is documented in [`../02-engineering/CONTEXT.md`](../02-engineering/CONTEXT.md) and reflected in the auto-tagging README's "Three consumers" + "Failure modes" sections.
